...
| Game Catalog completeness (4,919 games loaded; source Wikidata) | **KNOWN LIMIT** | Notable games only (≥ 7 Wikipedia editions at load time); niche titles and any platform Wikidata misses are not offered (fail safe). Extend with the exporter + loader (section 7t); better platform data would need IGDB (a key Mazen would create). No typo tolerance. |
