...
Application-owned Blob snapshotting and file-input DOM replacement are **NOT IMPLEMENTED** pending real-device evidence showing whether failure occurs before bitmap decode, in both decoders, or before a new `change` event. Existing lifecycle, decoder fallback, generation ownership, object-URL lifetime, and delayed input-reset fixes remain preserved. The complete diagnostic baseline is 49/49 tests. Slice 3A remains **AWAITING MAZEN'S ACCEPTANCE**; the next action is Mazen's exact reused-file reproduction through the diagnostic TESTING URL, followed by sharing the copied non-sensitive trace.
