...
**Public My Games: IMPLEMENTED — MANUALLY TESTED — ACCEPTED BY MAZEN (`PROJECT_HANDOFF.md` section 7w; accepted implementation checkpoint `006b97375a30d0be248d22e021b3588d673e1ee7`, including the global rank / stats privacy correction and the real Steam icon):** a visitor sees a compact MY GAMES section (at most six games + the true count) on the public GamID, **View all N games** opens the complete library with search over THAT identity's own games only, and every row (chevron) opens Game Details. Steam-discovered games show "Steam · Discovered", manual games "Manual · <platform>", a game that is both shows both on ONE row; nothing is ever labelled VERIFIED and Discord is not a game source. Three independent owner switches, all OFF by default: Show My Games, Show playtime, and Show ranks & stats, which is a GLOBAL public-GamID privacy control (it also gates the rank line of the public League card, not only Public My Games); no per-game switch; a switched-off value is absent from the server response, not hidden by CSS. Steam provenance uses the real Steam glyph (local, no remote asset) beside "Steam · Discovered". League keeps PROTOTYPE / UNVERIFIED. Not started: filters, Xbox / PlayStation, IGDB, Marvel stats, W1.
...
**Universal Social Identity Preview / Share Engine — architecture research done, Phase 0a Cloudflare hosting proof ACCEPTED, Phase 0b social-preview validation lab CLOSED (awaiting Mazen's acceptance before Phase 1)** — see `PROJECT_HANDOFF.md` section 7x. Sharing the real `@black` link in Discord produced no rich card; investigation found GitHub Pages answers `/@handle` with HTTP 404 and a client-side-only redirect, which crawlers never run. Chosen architecture: Cloudflare **Workers + Static Assets** (current Cloudflare guidance over Pages+Functions). Phase 0a: the accepted `dist/` deployed byte-identical to a temporary `gamid-testing-static.gamid.workers.dev` Worker (free plan), GitHub Pages untouched and still the link to share; `/@black` itself still 404s there, deliberately left for the future `/@*` Worker. Phase 0b: an isolated `gamid-social-lab` Worker with 17 synthetic (non-real-Intro) test pages, manually tested by Mazen on real Discord and WhatsApp. Confirmed: Discord shows a click-to-play video embed (never autoplay) for every video variant tried and needs a direct HTTP 200 (a 404 kills its preview; a redirect works but isn't the target); WhatsApp shows only a static poster/image, never plays video inline, and is slower to populate. Telegram, X, Facebook, iMessage, LinkedIn and Slack remain **UNTESTED** (Telegram testing was planned then explicitly cancelled — not required for acceptance). No domain, DNS, paid Cloudflare feature, Supabase, Auth/OAuth or Google Cloud change; the real `/@black` behavior, the accepted Intro/D3 pipeline and Production were not touched. Phase 1 (the real `/@*` dynamic Worker) is not started.
